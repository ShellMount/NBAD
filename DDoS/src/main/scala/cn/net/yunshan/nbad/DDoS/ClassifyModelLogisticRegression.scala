...
class ClassifyModelLogisticRegression extends ClassifyAlgorithm with Serializable{
  val modelName = "逻辑回归"
  var model: LogisticRegressionModel = null
  var model2:  LogisticRegression = null
  ...
  override def train(trainingData: RDD[LabeledPoint]) = {
    /**
      * 训练模型参数调整记录: 无
      */
    model = LogisticRegressionWithSGD.train(trainingData, Config.numIterations)

    //val filePath = Thread.currentThread().getContextClassLoader.getResource(Config.featureFileName).getPath
    //val model2 = LogisticRegression.load(filePath)
